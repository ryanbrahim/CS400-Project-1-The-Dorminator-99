...
public class HashTableMap <KeyType , ValueType> implements MapADT<KeyType , ValueType>{
	...
	public HashTableMap(int capacity) //Constructor 1 
	{  
		this.capacity = capacity; //This makes sure the maximum elements that Hash Table will store will be depend on the user 
		HashTable = new LinkedList[this.capacity];
		 for(int i = 0; i< HashTable.length ; i++ )  // This segment of the code is to declare an hash table and array of linked lists
		 {                                      
	...
	public HashTableMap ()  //Constructor 2
	{    
		this.capacity = 10; //This variable will make sure the maximum elements the HashTable will hold is 10
		 HashTable= new LinkedList[this.capacity];
		 for(int i = 0; i< HashTable.length ; i++ )  // This segment of the code is to declare an hash table and array of linked lists
		 {                                             
			 HashTable[i] = new LinkedList<Record>();   
				 ...
				 for(int j = 0; j< HashTable[i].size(); j++)
				   {
					  ...
					  {   
						  ...
						  temp[flag++] = new Record(key,value); // Stroring all the legitimate records in the array
	...
	public ValueType get(KeyType key) throws NoSuchElementException
	{
		int index = Math.abs((key.hashCode())%capacity);   //Calculate index values 
			...
			throw new NoSuchElementException("Element not found");  //Throws Exception when element is not found
